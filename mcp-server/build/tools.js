...
export async function handleToolCall(name, args) {
    switch (name) {
        ...
        case 'list_objects': {
            ...
            if (args.projectId)
                params.append('projectId', String(args.projectId));
            if (args.workspaceId)
                params.append('workspaceId', String(args.workspaceId));
        ...
        case 'list_items': {
            const params = new URLSearchParams();
            if (args.objectId)
                params.append('objectId', String(args.objectId));
            if (args.workspaceId)
                params.append('workspaceId', String(args.workspaceId));
            if (args.projectId)
                params.append('projectId', String(args.projectId));
        ...
        case 'create_context': {
            const ctxBody = { ...args, scope: args.scope || 'project' };
            ...
            if (ctxBody.projectId && !ctxBody.workspaceId) {
                ...
                try {
                    ...
                    const proj = projData.data?.find((p) => p.id === ctxBody.projectId);
                    if (proj?.workspaceId)
                        ctxBody.workspaceId = proj.workspaceId;
                }
                catch { /* ignore */ }
            }
        ...
        case 'list_connections': {
            const params = new URLSearchParams();
            if (args.scope)
                params.append('scope', String(args.scope));
            if (args.workspaceId)
                params.append('workspaceId', String(args.workspaceId));
            if (args.projectId)
                params.append('projectId', String(args.projectId));
