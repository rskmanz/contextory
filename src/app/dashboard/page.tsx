...
import { Context, ObjectType, ObjectItem, Project, ViewStyle, ContextType } from '@/types';

type ActiveTab = { type: 'context'; id: string } | { type: 'object'; id: string } | { type: 'item'; id: string };
type ModalName = 'addContext' | 'editContext' | 'addObject' | 'editObject' | 'deleteConfirm';
...
export default function DashboardPage() {
  ...
  const [objectViewScope, setObjectViewScope] = useState<'workspace' | 'project'>('workspace');
  const [contextViewScope, setContextViewScope] = useState<'workspace' | 'project'>('workspace');
  ...
  const viewLevel = 'global' as const;
  const workspace = '';
  const project = '';
  ...
  useEffect(() => {
    if (!activeTab) {
      if (globalContexts.length > 0) {
        setActiveTab({ type: 'context', id: globalContexts[0].id });
      } else if (globalObjects.length > 0) {
        setActiveTab({ type: 'object', id: globalObjects[0].id });
      }
    }
  }, [globalContexts, globalObjects, activeTab]);
  ...
  const handleRemoveObject = async (obj: ObjectType) => {
  ...
  const confirmDelete = async () => {
    const deletingItem = modal.getData<{ type: 'context' | 'object' | 'item'; item: Context | ObjectType | ObjectItem }>();
  ...
  const deletingData = modal.getData<{ type: 'context' | 'object' | 'item'; item: Context | ObjectType | ObjectItem }>();
            ...
            <div className="flex h-full w-full">
              {showWorkspaces && (
                <div className="w-48 flex-shrink-0 overflow-hidden">
                  <WorkspacesPanel
                    ...
                    workspace={workspace}
                    project={project}
                    currentWorkspace={null as unknown as any}
              ...
              <div className="flex-1 min-w-0 overflow-hidden">
                <ContextsObjectsPanel
                  ...
                  filteredContexts={globalContexts}
